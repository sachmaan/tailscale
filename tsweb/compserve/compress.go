...
// Content-Encoding, on Range requests, HEAD requests, non-200 statuses, or
// incompressible content types (see compressibleContentType).
//
...
type compressWriter struct {
	...
	head     bool   // request method is HEAD
	rangeReq bool   // request carried a Range header
...
// deadline and hijack support; Flush stays handled by the wrapper.
...
func (c *compressWriter) mayCompress() bool {
	if len(c.encs) == 0 || c.code != http.StatusOK {
		return false
	}
	if c.head || c.rangeReq {
		// HEAD responses have no body to compress, and range semantics
		// apply to the uncompressed representation.
		return false
	}
...
// already set by the handler, merged into a single header line.
